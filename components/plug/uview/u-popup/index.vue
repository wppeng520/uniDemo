		 ...
		 @touchmove.stop.prevent @tap.stop.prevent :style="[style]">
			<view class="u-mode-center-box" @tap.stop.prevent @touchmove.stop.prevent v-if="mode == 'center'" :style="[centerStyle]">
				<!-- <u-icon
					...
					:size="closeIconSize"
				></u-icon> -->
				<text class="my-icon u-close" :class="['u-close--' + closeIconPos]" @click="close" v-if="closeable">&#xe608;</text>
				<slot />
			</view>
			<block v-else>
				<slot />
			</block>
			<text class="my-icon u-close" :class="['u-close--' + closeIconPos]" v-if="mode != 'center' && closeable" @click="close">&#xe608;</text>
			<!-- <u-icon
	...
	export default {
		...
		computed: {
			...
			style() {
				...
				style.zIndex = this.zIndex ? this.zIndex : 10075;
